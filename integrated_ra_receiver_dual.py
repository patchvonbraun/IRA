...
# Generated: Sat Aug 21 20:03:27 2010
##################################################
...
class integrated_ra_receiver_dual(gr.top_block):

	def __init__(self, freq=1420.4058e6, gain=55, bandwidth=100.0e3, ratepsr=8000, dm=0.1, side='A', gcora=1.0, gcorb=1.0):
		...
		self.setisize = setisize = int(ad_rate/decim)/bin_width
		...
		self.connect((self.gr_add_xx_0, 0), (self.notch_filter_a, 0))
	...
	def set_ad_rate(self, ad_rate):
		...
		self.set_decim(int(self.ad_rate/self.ibandwidth))
		self.set_setisize(int(self.ad_rate/self.decim)/self.bin_width)
...
if __name__ == '__main__':
	...
	tb = integrated_ra_receiver_dual(freq=options.freq, gain=options.gain, bandwidth=options.bandwidth, ratepsr=options.ratepsr, dm=options.dm, side=options.side, gcora=options.gcora, gcorb=options.gcorb)
	tb.run()
