...
# Generated: Sat Mar 28 17:40:53 2009
##################################################
...
class integrated_ra_receiver_simmed(gr.top_block):
	...
	def __init__(self, freq=1420.4058e6, gain=55, setisize=4096, ratepsr=8000, ratetp=100, decim=16, g_cor_a=1.0, g_cor_b=1.0):
		...
		# Blocks
		##################################################
		self.gain_block = gr.multiply_const_vcc((complex(gain,gain), ))
		self.gr_add_vxx_0 = gr.add_vcc(1)
		...
		self.gr_noise_source_x_0 = gr.noise_source_c(gr.GR_GAUSSIAN, 1, os.getpid()/42)
		...
		self.gr_throttle_0 = gr.throttle(gr.sizeof_gr_complex*1, setisize)
		...
		self.connect((self.gr_add_vxx_0, 0), (self.gr_stream_to_vector_0, 0))
		self.connect((self.gr_add_vxx_0, 0), (self.gr_complex_to_mag_squared_0_4_0_1, 0))
		self.connect((self.gr_multiply_const_vxx_0, 0), (self.gr_add_vxx_0, 0))
		self.connect((self.gr_multiply_const_vxx_0_0, 0), (self.gr_add_vxx_0, 1))
		...
		self.connect((self.gr_throttle_0, 0), (self.notch_filter_b, 0))
		self.connect((self.gr_throttle_0, 0), (self.notch_filter_a, 0))
		self.connect((self.gr_noise_source_x_0, 0), (self.gain_block, 0))
		self.connect((self.gain_block, 0), (self.gr_throttle_0, 0))
	...
	def set_gain(self, gain):
		self.gain = gain
		self.gain_block.set_k((complex(gain,gain), ))
...
if __name__ == '__main__':
	...
	tb = integrated_ra_receiver_simmed(freq=options.freq, gain=options.gain, setisize=options.setisize, ratepsr=options.ratepsr, ratetp=options.ratetp, decim=options.decim, g_cor_a=options.g_cor_a, g_cor_b=options.g_cor_b)
	tb.start()
	NOTCH_SIZE=64
